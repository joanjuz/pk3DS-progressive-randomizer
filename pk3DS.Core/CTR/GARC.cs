...
public static class GARC
{
    ...
    public static int PackGARC(string folderPath, string garcPath, int version, int bytesPadding)
    {
        ...
        try { DeleteFileWithRetry(garcPath); }
        catch { }
    ...
    private static void DeleteFileWithRetry(string path)
    {
        ...
        for (int i = 0; i < attempts; i++)
        {
            ...
            catch (System.IO.IOException) when (i + 1 < attempts)
            {
                System.Threading.Thread.Sleep(delayMs);
            }
            catch (System.UnauthorizedAccessException) when (i + 1 < attempts)
            {
                System.Threading.Thread.Sleep(delayMs);
            }
        }
