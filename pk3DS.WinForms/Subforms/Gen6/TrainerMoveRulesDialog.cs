...
public static class TrainerMoveRulesDialog
{
    public static bool Edit(IWin32Window owner, ref List<TrainerMoveRule> rules)
    {
        ...
        var note = new Label
        {
            ...
            Text = "Strong Stat compares base Attack vs Sp. Attack. If the difference is greater than Tolerance, the Pokémon uses only Physical or Special damaging moves. If the difference is within Tolerance, both categories are allowed. Allow Status is enabled by default. Better Movesets forces the improved moveset builder for Use-checked trainers even when the global Better Movesets checkbox is off. Min Power 0 disables minimum power filtering. Smart Items gives competitive held items only to Use-checked trainers. EVs -1 disables EV override; otherwise the selected value is applied to all EV stats of every Pokémon in that trainer battle. The EV buttons apply to every trainer with Use checked, regardless of selected rows.",
        };
